...
def get_secrets(credentials: AWSCredentials_SecretsManager) -> dict:
    ...
    get_secret_value_response = client.get_secret_value(
        SecretId=credentials.secret_name
    )
...
async def validate_sqs_event_record(
    event: dict, context: Any, db_writer: Callable = None
) -> Union[Dict, str]:
    ...
    if "Records" not in event:
        ...
        if db_writer is not None:
            await db_writer(
                error_type="GENERAL_SQS_MESSAGE_ERROR",
        ...
        if len(event["Records"]) == 0:
            print(
                f"'Records' attribute is empty in the provided event. Finishing task..."
            )
            if db_writer is not None:
                await db_writer(
                    error_type="GENERAL_SQS_MESSAGE_ERROR",
            ...
            return get_error_return(
                error=f"'Records' attribute is " f"empty in the provided event."
            )

        if "body" not in event["Records"][0]:
            print(
                f"'Records' attribute does not have 'body' attribute "
        ...
        if "receiptHandle" not in event["Records"][0]:
            ...
            return get_error_return(
                error=f"'Records' attribute does not have 'receiptHandle' "
                f"attribute in the provided event."
            )
